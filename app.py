    ...
    if not df.empty:
        ...
        if izabrani_broj:
            ...
            if not rezultat.empty:
                ...
                # --- TAB 2: KULTURE (POVEZIVANJE PREKO NAZIVA) ---
                with tab2:
                    st.write(f"Tražim kulture za model: **{model_iz_opreme}**")
                    try:
                        conn = get_conn()
                        # Izvlačimo celu tabelu kulture da bismo je "upeglali" u Pythonu
                        df_sve_kulture = pd.read_sql("SELECT * FROM kulture_opsezi", conn)
                        conn.close()

                        if not df_sve_kulture.empty:
                            # 1. Standardizacija kolona tabele kulture
                            df_sve_kulture.columns = [c.strip().lower() for c in df_sve_kulture.columns]
                            
                            # 2. Filtriranje: Tražimo poklapanje naziva (brišemo razmake sa obe strane)
                            df_k = df_sve_kulture[df_sve_kulture['naziv_proizvodjac'].astype(str).str.strip() == model_iz_opreme]

                            if not df_k.empty:
                                # 3. Čišćenje: Izbacujemo redove koji su naslovi (npr. "kultura" ili "nan")
                                df_k = df_k[df_k['kultura'].astype(str).str.lower() != 'kultura']
                                df_k = df_k[df_k['kultura'].notna()]
                                
                                # 4. Odabir bitnih kolona i čišćenje praznina
                                ...
                                if not prikaz.empty:
                                    ...
                                    st.info("Pronađeni podaci su prazni.")
                            else:
                                st.warning(f"U tabeli 'kulture_opsezi' nema poklapanja za: '{model_iz_opreme}'")
                                st.info("Savet: Proveri da li se naziv u slovo podudara u obe tabele (uključujući razmake između reči).")
                        else:
                            st.error("Tabela 'kulture_opsezi' je prazna u bazi.")
                ...
                st.warning("Instrument sa tim inventarnim brojem nije pronađen.")
        else:
            st.info("👈 Ukucaj inventarski broj u polje sa leve strane da otvoriš Matični Karton.")
    else:
