...
def get_conn():
    return pymysql.connect(
        ...
        port=27698, 
        database="defaultdb",
        cursorclass=pymysql.cursors.DictCursor,import streamlit as st
import pandas as pd
import pymysql
from datetime import datetime
        ...
        if izabrani_broj:
            ...
            if not rezultat.empty:
                ...
                with tab2:
                    ...
                    try:
                        conn = get_conn()
                        # ISPRAVLJENO: Dodata dva %% da Python ne bi prijavio grešku sa argumentima
                        query_k = """
                        ...
                        st.error(f"SQL Greška u Tabu 2: {e}")
...
try:
    df = get_working_data()
    if not df.empty:
        # --- ČIŠĆENJE GLAVNE TABELE ---
        # Ovde je bila greška - popravljeno sa .str.lower()
        if 'inventarni_broj' in df.columns:
            df = df[df['inventarni_broj'].astype(str).str.lower() != 'inventarni_broj']
        ...
        if izabrani_broj:
            ...
            if not rezultat.empty:
                instrument = rezultat.iloc[0] # Uzimamo prvi red podataka
                model_instrumenta = str(instrument.get('naziv_proizvodjac', '')).strip()
                inv_broj_str = str(izabrani_broj)
