                    ...
                    if is_admin:
                        with st.form(f"f_{tab_ime}"):
                            ...
                            if st.form_submit_button(f"Sačuvaj {nasl}"):
                                ...
                                conn = get_conn(); cur = conn.cursor()
                                if nasl == "Servis": cur.execute(f"INSERT INTO {tab_ime} (inventarni_broj, {kol_dat}, opis_kvara, broj_uverenja, vazi_do) VALUES (%s,%s,%s,%s,%s)", (izabrani_broj, d_rada, opis, br_dok, novi_rok.date()))
                                else: cur.execute(f"INSERT INTO {tab_ime} (inventarni_broj, {kol_dat}, broj_uverenja, vazi_do) VALUES (%s,%s,%s,%s)", (izabrani_broj, d_rada, br_dok, novi_rok.date()))
                                cur.execute("UPDATE oprema SET vazi_do = %s WHERE inventarni_broj = %s", (novi_rok.date(), izabrani_broj))
                                conn.commit(); conn.close(); st.rerun()
            with t4:
                model_ap = ins.get('naziv_proizvodjac')
                if model_ap:
                    df_k = run_query("SELECT * FROM kulture_opsezi WHERE naziv_proizvodjac = %s", (model_ap,))
                    if not df_k.empty: st.dataframe(df_k, use_container_width=True)
                    else: st.info(f"Nema kultura za model {model_ap}")
