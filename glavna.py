...
# Inicijalizacija stanja sesije (da aplikacija pamti ko je ulogovan)
...
# CSS za sakrivanje sidebara na login ekranu
st.markdown("""
    <style>
...
def proveri_korisnika(username, password):
    # Provera u tabeli zaposleni
    query = "SELECT ime, prezime, is_premium FROM zaposleni WHERE korisnicko_ime = %s AND lozinka = %s"
    res = run_query(query, (username, password))
    if not res.empty:
        return res.iloc[0] # Vraća prvi pronađen red
    return None

# 3. SADRŽAJ POČETNE STRANE
def prikazi_pocetnu():
    st.title("👋 Dobrodošli u BV Web App")
    st.write("Sistem za centralnu evidenciju opreme i laboratorijsku kontrolu.")
    st.markdown("---")
    ...
    if st.session_state['ulogovan']:
        ...
        with col2:
            st.subheader("🔐 Prijava na sistem")
            ...
            if st.button("PRIJAVI SE", use_container_width=True, type="primary"):
                ...
                if podaci is not None:
                    ...
                    st.session_state['ime_korisnika'] = f"{podaci['ime']} {podaci['prezime']}"
                    st.success("Uspešna prijava!")
                    st.rerun()
                else:
                    st.error("Neispravni podaci. Pokušajte ponovo.")

# 4. DEFINISANJE STRANICA (Sve su u pages/ osim glavne)
...
# Ako korisnik nije ulogovan, vidi samo Login (Početnu)
if st.session_state['ulogovan']:
